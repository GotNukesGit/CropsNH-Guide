...
## September 6, 2026 — Guide must-fix + likely-wrong pass (`cropsnh-2.0.114.jar`)

Read-only claims were re-checked against CFR output of first-party 2.0.114 classes (`TileEntityCropSticks`, `CropCard`, `MutationRegistry`, `MutationPool`, `CropRubyne`, `ConfigurationHandler`, `MTEIndustrialFarm`). No IC2 tables.

- **Rubyne soil:** `CropRubyne.getSoilTypes()` returns `CropsNHSoilTypes.stone`. `js/data.js` had `farmland`; corrected to `stone`. Guide soil counts are now 63 farmland / 61 stone / 28 dirt-grass.
- **WeedEx / weeds:** incoming `transferDisease()` burns 2 or is blocked by `Resistance > random(0–30)`. Empty-stick `spawnWeed()` burns 5. `spreadsWeeds()` is mature + Growth > 24 + Resistance < Growth; Resistance ≥ Growth alone stops outward spread. Removed the false “WeedEx AND R≥G for both directions / incoming costs 5” wording.
- **Nether:** liked-tag bonus, not a hard location lock.
- **Alt-seed count:** 83 total (5 vanilla + 78 others), not 85 others.
- **Pool sizes (after 2.0.114 pool override):** EDIBLE 50, YELLOW 14, FLOWER 26.
- **Deterministic graph:** 13 crops from the five vanilla alt-seeds only; 165 of 179 if every alt-seed crop is a starter.
- **Torchberry pools:** `BERRY` + `EMISSIVE` + `TWILIGHT_FOREST` + `YELLOW` — not a single `BERRY` pool.
- **Canyon table:** unique-best-served recomputed from current liked tags + `BIOMES` — Canyon 59/45, Fungi Forest 47/28, Crag 12/9, FrozenOcean 13/6. Footnote 93 / 152 coverage numbers were already correct.
- **FloweringVine / Plumbilia:** still zero tag overlap with Canyon / Fungi Forest / Crag / FrozenOcean.
- **Industrial Farm modes** 0 input / 1 farm / 2 output and the Crop Manager “does not auto-replant” note left as written.
- Dropped leftover “GTNH 2.9 Beta 2” branding on `index.html` and `production.html`.
